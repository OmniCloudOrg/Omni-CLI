...
impl From<&SshHost> for SshHostDisplay {
    fn from(host: &SshHost) -> Self {
        SshHostDisplay {
            ...
            identity_file: host.identity_file.clone().unwrap_or_else(|| "-".to_string()),
...
impl PremiumUI {
    pub async fn init_environment(&self) -> Result<()> {
        ...
        let config = if Path::new(&config_path).exists() {
            ...
            let config_json = fs::read_to_string(&config_path).context("Failed to read configuration file")?;
            let config: CloudConfig = serde_json::from_str(&config_json).context("Failed to parse configuration")?;
            ...
            println!("{}", style("This wizard will help you configure your self-hosted cloud environment.").dim());
            ...
            let cloud_name: String = Input::with_theme(&self.theme)
                ...
                .default(format!("{}-cloud", company_name.to_lowercase().replace(" ", "-")))
                .interact_text()?;

            // Region selection
            let regions = vec!["us-east", "us-west", "eu-west", "eu-central", "ap-southeast", "custom"];
            ...
            println!("{}", style("Configure SSH hosts for your cloud environment").dim());
            ...
            loop {
                ...
                if !ssh_hosts.is_empty() {
                    ...
                    let display_hosts: Vec<SshHostDisplay> = ssh_hosts.iter()
                        .map(SshHostDisplay::from)
                        .collect();
            ...
            println!("{}", style(format!("✅ Configuration saved to {}", config_path)).green());
            ...
            println!("SSH Hosts: {}", style(config.ssh_hosts.len()).green());
            println!("Monitoring: {}", if config.enable_monitoring { style("Enabled").green() } else { style("Disabled").yellow() });
            println!("Backups: {}", if config.enable_backups { style("Enabled").green() } else { style("Disabled").yellow() });
            
            if config.enable_backups {
                println!("Backup Retention: {} days", style(config.backup_retention_days).green());
            }
        ...
        println!("{}", style(format!("Setting up OmniOrchestrator for {} cloud environment", config.cloud_name)).dim());
        
        // Check if there are SSH hosts configured
        if config.ssh_hosts.is_empty() {
            println!("{}", style("No SSH hosts configured. Cannot bootstrap OmniOrchestrator.").yellow());
            return Ok(());
        }
        ...
        println!("\n{}", style("✨ Environment initialization completed!").green().bold());
        println!("{}", style("Your OmniOrchestrator cloud environment is ready.").dim());
        println!("{}", style("You can now deploy applications with 'omni deploy'.").dim());
    ...
    async fn bootstrap_orchestrator(&self, config: &CloudConfig) -> Result<()> {
        ...
        println!("\n{}", style(format!("Installing OmniOrchestrator on {} hosts...", total_hosts)).cyan());
        ...
        println!("\n{}", style(format!("✅ OmniOrchestrator installed on all {} hosts", total_hosts)).green().bold());
        ...
        if config.enable_backups {
            ...
            for i in 0..100 {
                ...
                match i {
                    ...
                    60 => pb.set_message(format!("Setting {} day retention policy", config.backup_retention_days)),
    ...
    async fn install_on_host(&self, host: &SshHost, config: &CloudConfig, is_bastion: bool) -> Result<()> {
        ...
        for (step, duration) in steps.iter() {
            ...
            for i in 0..*duration {
                ...
                match i {
                    5 => pb.set_message(format!("{} (connecting to {}@{}:{})", step, host.username, host.hostname, host.port)),
        ...
        println!("{}", style(format!("✅ OmniOrchestrator installed on {}", host.name)).green());
        Ok(())
    }
    ...
    pub async fn list_ssh_hosts(&self) -> Result<()> {
        ...
        if !Path::new(config_path).exists() {
            println!("{}", style("No cloud configuration found. Run 'omni init' first.").yellow());
            return Ok(());
        }
        ...
        if config.ssh_hosts.is_empty() {
            println!("{}", style("No SSH hosts configured. Run 'omni init' to add hosts.").yellow());
            return Ok(());
        }
        
        println!("\n{}", style("📡 Configured SSH Hosts").cyan().bold());
        println!("Cloud: {} ({})", style(&config.cloud_name).green(), &config.region);
        
        let display_hosts: Vec<SshHostDisplay> = config.ssh_hosts.iter()
            .map(SshHostDisplay::from)
            .collect();
    ...
    pub async fn orchestrator_status(&self) -> Result<()> {
        ...
        if config.ssh_hosts.is_empty() {
            println!("{}", style("No hosts configured. Run 'omni init' to configure hosts.").yellow());
            return Ok(());
        }
        ...
        println!("Backups: {}", if config.enable_backups { style("Enabled").green() } else { style("Disabled").yellow() });
        if config.enable_backups {
            println!("  Retention: {} days", style(config.backup_retention_days).green());
        ...
        println!("- {}: Restart a service", style("omni service restart <host> <service>").yellow());
        println!("- {}: View detailed logs", style("omni logs <host> <service>").yellow());
        println!("- {}: Trigger immediate backup", style("omni backup now").yellow());
